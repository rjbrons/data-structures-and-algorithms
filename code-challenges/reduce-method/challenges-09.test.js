...
const reversedString = arr => {
  let myArr = arr.split('');
  return myArr.reduce((accum, curr) => {
    return curr + accum;
  });
};
...
const countNumberOfChildren = arr => {
  return arr.reduce((accum, curr) => {
    return accum + (curr.children ? curr.children.length : 0);
  }, 0);
};
...
const countPrimeNumbers = arr => {
  return arr.reduce((accum, curr) => (isPrime(curr) ? accum + 1 : accum), 0);
};
...
//for each element if obj.name === str return obj.
...
const extractChildren = arr => {
  let re = /a/;
  let newArr = arr.filter(obj => re.test(obj.name));

  return newArr.reduce((accum, curr) => {
    if (curr.children) {
      curr.children.forEach(c => accum.push(c));
    }
    return accum;
  }, []);
};
