...
const hasChildrenValues = (arr, character) => {
  for (let i in arr) {
    ...
    if (vals[0] === character) {
      if (vals[2].length > 0) return true;
    }
  }
  return false;
};
...
const totalCharacters = arr => {
  ...
  arr.forEach(char => {
    ...
    if (char.spouse) count += 1;
    if (char.children) count += char.children.length;
  });
  return count;
};
...
const houseSize = arr => {
  ...
  arr.forEach(house => {
    ...
    let count = 0;
    if (house.name) count += 1;
    if (house.spouse) count += 1;
    if (house.children) count += house.children.length;
    obj.members = count;
    sizes.push(obj);
  });
  return sizes;
...
const houseSurvivors = arr => {
  ...
  arr.forEach(house => {
    ...
    let obj = {};
    obj.house = house.house;
    if (!house.spouse || deceasedSpouses.includes(house.spouse));
    else count += 1;
    if (house.name) count += 1;
    if (house.children) count += house.children.length;
    obj.members = count;
