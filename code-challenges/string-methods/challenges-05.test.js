...
const listFoods = recipe => {
  ...
  recipe.ingredients.forEach(e =>
    result.push(e.slice(e.indexOf(' ', e.indexOf(' ') + 1) + 1))
  );
  return result;
};
...
const stepActions = recipe => {
  let result = [];
  recipe.steps.forEach(e => result.push(e.slice(0, e.indexOf(' '))));
  return result;
};
...
const removeLastCharacters = (str, numberOfCharacters) => {
  return numberOfCharacters < 0 ? str : str.slice(0, '-' + numberOfCharacters);
};
...
const extractVowels = str => {
  ...
  let cons = [];
  let vow = [];
  str.split('').forEach(ch => (re.test(ch) ? vow.push(ch) : cons.push(ch)));
  return [cons.join(''), vow.sort().join('')];
};
